...
_libjv2.default.checkUpload = function (setting) {
  var vm = setting.vm;
  ...
  var maxSize = _libjv2.default.toByteUnit(setting.maxSize || "1M"); //KB

  var checkFileType = function checkFileType(fileName, fileType) {
  ...
  if (chkItem) {
    _libjv2.default.error("文件类型不允许，只能上传: " + chkItem.type + "类型！");
    return false;
  }
  ...
  if (maxCount > 1 && vmUpload.fileList.length > maxCount) {
    _libjv2.default.error("超出上传文件的数量限制，最多上传: " + maxCount + " 个文件！");
    return false;
  }
  ...
  return new Promise(function (resolve, reject) {
    //7.检查Md5,上传
    var doUpload = function doUpload(imgBase64) {
      ...
      if (file.size > maxSize) {
        _libjv2.default.error("文件太大，超出许可范围！");
        reject({});
        return;
      }

      _libjv2.default.getFileMd5(file).then(function (md5) {
        //8.检查服务器文件的 Md5值。
        vm.$http.post("/sys/fileCheckUpload", { md5: md5 }).then(function (res) {
          ...
          if (file.size > _libjv2.default.toByteUnit("10MB")) {
            _libjv2.default.info("正在上传，请稍等！");
          }
    ...
    //5. 真正的处理：压缩，Md5
    var compressImg = function compressImg(imgDataBase64) {
