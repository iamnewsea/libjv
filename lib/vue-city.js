...
_libjv2.default.citys.loadChildCitys = function (code, ajax, loaded) {
  code = parseInt(code);
  if (code % 100) return;
  ...
  ajax.post(_libjv2.default.citys.url, { code: code }).then(function (res) {
...
_libjv2.default.citys.confirm = function (code, ajax, loaded) {
  if (!code) return;
  code = parseInt(code);
  var city = _libjv2.default.citys.findByCode(code);
  ...
  var level = _libjv2.default.citys.getLevel(code);
  if (level >= 2) {
    _libjv2.default.citys.loadChildCitys(parseInt(code / 10000) * 10000, ajax, function (it) {
      if (level == 3) {
        _libjv2.default.citys.loadChildCitys(parseInt(code / 100) * 100, ajax, loaded);
      }
    });
  }
};
