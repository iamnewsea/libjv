...
jv.citys.forEach(it => {
  if (it.c % 10000) {
    jv.citys.zhixia.push(it.c / 10000);
  }
});

jv.citys.isZhixia = function (code) {
  return jv.citys.zhixia.indexOf(code / 10000) >= 0;
}
...
jv.citys.findByCode = function (code) {
  ...
  var findSubOne = function (datas, code, level, maxLevel) {
    ...
    var levelCode = code / chuShu * chuShu;
  ...
  if (level == 1 && iszhi) {
    return jv.citys.filter(it => it.c / 10000 == code / 10000)[0];
  }
...
jv.citys.confirm = function (code, ajax, loaded) {
  ...
  var level = jv.citys.getLevel(code);
  if (level >= 2) {
    jv.citys.loadChildCitys(code / 10000 * 10000, ajax, it => {
      if (level == 3) {
        jv.citys.loadChildCitys(code / 100 * 100, ajax, loaded);
      }
    });
  }
}
...
jv.citys.getEachCitys = function (code) {
  ...
  for (var i = 1; i <= level; i++) {
    ...
    var city = jv.citys.findByCode(code / chuShu * chuShu);
    if (!city) {
      if (i == 1) {
        continue;
      }
