...
//规则：如果没有定义，按上级配置。 如果最上级没有配置，按false
  router.options.routes.recursion(function (it) {
    ...
    if (it.children) {
      ...
      for (var index in  it.children) {
        var item = it.children[index];
...
jv.initApp = function (vueProtype) {
  ...
  //file,需要 url ,type
  vueProtype.Preview = function (p) {
    if (!p || !p.url) {
      return;
    }
    _vue._data.Preview_Image_Show = true;
    if (p.type == "video") {
      _vue._data.Preview_Image = "";
      _vue._data.Preview_Video = p.url;
    }
    else {
      _vue._data.Preview_Image = p.url;
      _vue._data.Preview_Video = "";
    }
  };
