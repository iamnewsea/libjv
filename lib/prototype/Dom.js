...
(function () {
    ...
    var query2Json = function query2Json(query) {
        var ret = {};
    ...
    var loadHasjJson = function loadHasjJson() {
        // http://blog.csdn.net/lvjin110/article/details/37663067
    ...
    //vue 使用了 pushState
    var pushState_ori = history.pushState;
    ...
    location.json2href = function () {
        return location.protocol + "//" + location.host + location.pathname + "?" + Object.keys(location.json).map(function (it) {
            return it + "=" + encodeURIComponent(location.json[it]);
        }).join("&");
    };
