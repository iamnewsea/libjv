...
jv.citys.recursion(function (it) {
  return it.s
}, function (it) {
  if (it.c % 100) return;
...
jv.citys.findByCode = function (code) {
  ...
  if (level == 1 && iszhi) {
    return jv.citys.filter(function (it) {
...
jv.citys.confirm = function (code, ajax, loaded) {
  ...
  var level = jv.citys.getLevel(code);
  if (level >= 2) {
    jv.citys.loadChildCitys(parseInt(code / 10000) * 10000, ajax, function (it) {
      if (level == 3) {
