...
_libjv2.default.initApp = function (vue) {
    _libjv2.default.Vue = vue;
    //关闭环境给出的提示.
...
_libjv2.default.getAjaxCacheKey = function (config) {
...
_libjv2.default.initAxios = function (axios) {
    ...
    var translateDate = function translateDate(value, callback) {
        if (!value) return;
        var dtValue,
    ...
    axios.interceptors.response.use(function (response) {
        ...
        if (json && json.msg) {
            _libjv2.default.error(json.msg);
        ...
        var cacheKey = _libjv2.default.getAjaxCacheKey(response.config);
        //最多保存500个缓存记录
        if (cacheKey && json.data && _libjv2.default.cache_db.length < 500) {
            _libjv2.default.cache_db[cacheKey] = { data: JSON.stringify(json), cacheAt: new Date().totalSeconds };
        }
        return response;
    }, function (error) {
        if (!error.response) {
    ...
    axios.post = function (url, data, config) {
        ...
        var cacheKey = _libjv2.default.getAjaxCacheKey(Object.assign({}, config, {
            method: "post",
            url: _libjv2.default.ajax.defaults.baseURL + url,
            data: _libjv2.default.ajax.defaults.transformRequest(data)
        }));
        if (cacheKey in _libjv2.default.cache_db) {
            var cacheData = _libjv2.default.cache_db[cacheKey];
