...
(function () {
    ...
    _vueInit2.default.chk_types = {
        "float": function float(chk_body, value) {
            return (/^[+-]?[0-9]+.?[0-9]*$/.test(value)
            );
        },
        "int": function int(chk_body, value) {
            return (/^[+-]?[0-9]+$/.test(value)
            );
        },
        "date": function date(chk_body, value) {
            return (/^\d{4}[-/]([01]?\d|2[0-4])[-/]([0-2]?\d|3[0-1])$/.test(value)
            );
        },
        "date-time": function dateTime(chk_body, value) {
            return (/^\d{4}[-/]([01]?\d|2[0-4])[-/]([0-2]?\d|3[0-1]) ([01]?\d|2[0-3]):[0-5]?\d:[0-5]?\d$/.test(value)
            );
        },
        "time": function time(chk_body, value) {
            return (/^([01]?\d|2[0-3]):[0-5]?\d:[0-5]?\d$/.test(value)
            );
        },
        "email": function email(chk_body, value) {
            return (/^([\w-])+@([\w-])+(\.[\w-]{1,})$/.test(value)
            );
        },
        //名称
        "name": function name(chk_body, value) {
            return (/^[\w\d]+$/.test(value)
            );
        },
        //*号必填
        "*": function _(chk_body, value) {
            if (!value.length) {
                return false;
            }
            return true;
    ...
    // };
    //
    var getVueTooltipFromUp = function getVueTooltipFromUp(vueDom, deep) {
        if (!vueDom) return;
        deep = deep || 0;
    ...
    //-------------------------------------------------------------------------------------
    //找到后，就不再查找子元素了。
    var getAllVuesChkDom = function getAllVuesChkDom(vueDom) {
        var ret = [];
        ...
        vueDom.$children.forEach(function (v) {
    ...
    var getVueData = function getVueData(component) {

        var vnode = component.$vnode,
            vdata = vnode.data,
            ret = {},
            data = vnode.context._data;
        if (vdata && vdata.model && vdata.model.expression) {
            ...
            var ret = _vueInit2.default.evalExpression(data, vdata.model.expression);
            if (ret.ok) {
    ...
    var chk_item = function chk_item(chk_dom) {
        ...
        var _getVueData = getVueData(chk_dom),
            value = _getVueData.value,
            data = _getVueData.data;
        ...
        if (chk_type == ":") {
            ...
            var r2 = _vueInit2.default.chk_types[chk_type](chk_body, value);
            ...
            if (chk_body) {
                // chk_body.split("&")
                var r = _vueInit2.default.chk_range(chk_type, chk_body, value);
                if (r === false) {
                    ret.result = r;
                    ret.detail = "范围不正确";
                    return ret;
    ...
    _vueInit2.default.chk_vue_dom = function (container, singleShow) {
        var index = 0,
            ret = true,
